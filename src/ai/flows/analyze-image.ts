...
const AnalyzeImageOutputSchema = z.object({
  description: z
    .string()
    .describe(
      "A prompt of the user interface image and its design characteristics."
    ),
});
...
const analyzeImagePrompt = ai.definePrompt({
  ...
  prompt: `You are an expert Prompt-Generator UI design. Examine the provided image of a user interface and provide a detailed Prompt for UI design description of its notable UI design characteristics, such as color themes (give in Hex code), border styles, background color, font color, font type, feature (e.g Filter, Search form etc), and layout. Intelligently assess its characteristics 'notable' from a UI design perspective. Please uses following format:
...
NOTE: Response with Markdown format.
